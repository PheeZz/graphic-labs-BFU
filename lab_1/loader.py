...
class easy_shapes:
    ...
    def draw(self, event):
        ...
        draw_image.ellipse(
            [x1, y1, x2, y2], fill=self.brush_color, outline=self.brush_color)
    ...
    def square(self):
        self.canvas.create_rectangle(
            self.x, self.y, self.x + self.brush_size, self.y + self.brush_size, fill=self.brush_color, outline=self.brush_color)
    ...
    def angle_90(self):
        self.canvas.create_arc(self.x, self.y, self.x + self.brush_size, self.y + self.brush_size,
                               start=90, extent=90, fill=self.brush_color, outline='black')

    def traffic_light(self):
        # draw 3 circles with different colors
        self.canvas.create_oval(self.x, self.y, self.x + self.brush_size,
                                self.y + self.brush_size, fill='red', outline='black')

        self.canvas.create_oval(self.x, self.y + self.brush_size, self.x + self.brush_size,
                                self.y + 2*self.brush_size, fill='yellow', outline='black')

        self.canvas.create_oval(self.x, self.y + 2*self.brush_size, self.x +
                                self.brush_size, self.y + 3*self.brush_size, fill='green', outline='black')
